...
> "一人公司" Trae版 — 注意力友好的多Agent协作框架 + 动态状态机编排器
## 演示视频
...
## v2.1 新特性：宪法按阶段拆分 + 多维度质量门

v2.0 引入项目类型自适应；v2.1 聚焦宪法体系的工程化重构：

- **宪法按阶段拆分**：41 条规则分散到 8 个阶段文件中，每个阶段 ≤7 条，AI 不会一次性接收全部规则
- **C40 多维度质量门**：代码异味、复杂度、dead code、安全漏洞、性能瓶颈 — 5 个维度统一在 TEST_GATE 验证
- **C41 攻击注入机制**：must_fix ↔ attacks 绑定，从 Module 3 复盘自动提取攻击维度
- **审计器多文件检查**：`harness_auditor.py` 覆盖 6 个宪法文件而非 1 个
...
- **注意力控制**：每个子Agent规则≤7条，上下文≤400 token
- **信息瓶颈**：模块间只传递执行清单JSON (~200 token)
- **全程3子Agent**：符合Trae并发限制，保持效率
- **编排器铁门**：动态状态机，每步必须通过闸门才推进
...
| 维度 | v1.0 | v1.5 | v2.0 | **v2.1** |
|------|------|------|------|------|
| **编排器** | 无 | 15 步状态机 | 动态状态机 | 动态状态机 |
| **闸门数量** | 2 个 | 4 个 | 5 个（+ VISUAL_GATE） | 5 个 |
| **宪法规则** | 25 条 | 39 条 | 39 条 | **41 条（+C40-C41）** |
| **规则加载** | 全量 | 全量 | 全量 | **按阶段拆分，每阶段 ≤7 条** |
| **Agent 阵容** | 固定 8 个 | 固定 8 个 | 按项目类型自适应 | 按项目类型自适应 |
| **项目模板** | 无 | 无 | 8 个 YAML | 8 个 YAML |
| **非代码支持** | 纯代码 | 纯代码 | 视频/设计/动画 | 视频/设计/动画 |
| **审计器** | 无 | 单文件 | 单文件 | **6 文件并行检查** |
| **流程保证** | ~12% | 100% | 100% | 100% |
...
Trae Harness会自动：
1. 判定复杂度 + 识别项目类型 + 配置攻击维度（L1/L2/L3）
2. 拆分维度并并行调研
3. 红蓝辩论（同时1维度，3角色并行）
4. 审核裁决（3审核员并行，含代码建议）
5. 生成规范并用户确认
...
| 阶段 | 文件 | 规则数 | 包含规则 |
|------|------|:---:|------|
| Phase 0 | phase-0-complexity.md | 6 | C01-C04, C26, C34 |
| Phase 1 | phase-1-research.md | 3 | C05-C07 |
| Phase 2 | phase-2-debate.md | 5 | C08-C11, C30 |
| Phase 3 | phase-3-review.md | 4 | C12-C15 |
| Phase 4 | phase-4-spec.md | 3 | C16-C18 |
| Phase 5 | phase-5-handover.md | 3 | C19-C21 |
| Module 2 Steps | module-2-steps.md | 4 | C22-C25 |
| Module 2 Gates | module-2-test-gates.md | 6 | C27-C29, C32-C33, C40 |
| 跨阶段 | management-rules.md | 7 | C31, C35-C39, C41 |

> 每个阶段 Agent 只加载 ≤7 条规则，控制注意力消耗在 400 token 以内。

### v2.1 新增规则

| ID | 规则 | 来源 |
|:--:|------|:--:|
| C40 | 多维度质量门：代码异味、复杂度、dead code、安全漏洞、性能瓶颈 — 5 维度统一在 TEST_GATE 验证 | v12-v16 复盘 |
| C41 | 攻击维度注入机制：must_fix ↔ attacks 绑定，从 Module 3 复盘自动提取攻击维度 | v12-v16 复盘 |
...
| `harness_auditor.py` | AUDIT_GATE | 最终审计：6 文件并行宪法检查 + 产出物完整性 |
...
> 模型: **DeepSeek V4 Pro** | 实验: **v1-v16 共 16 轮 A/B 对照** | 详见 [trae-harness-experiments](https://github.com/zzzkeepdd/trae-harness-experiments)

### 代码质量提升（13 个维度，全部 WIN）
...
│   │   ├── management-rules.md          # C31,C35-C39,C41 跨阶段管理
│   │   ├── rules-by-phase/              # 按阶段分散加载（v2.1）
│   │   │   ├── phase-0-complexity.md    # C01-C04,C26,C34
│   │   │   ├── phase-1-research.md      # C05-C07
│   │   │   ├── phase-2-debate.md        # C08-C11,C30
│   │   │   ├── phase-3-review.md        # C12-C15
│   │   │   ├── phase-4-spec.md          # C16-C18
│   │   │   ├── phase-5-handover.md      # C19-C21
│   │   │   ├── module-2-steps.md        # C22-C25
│   │   │   └── module-2-test-gates.md   # C27-C29,C32-C33,C40
│   │   ├── full-constitution.md         # 索引+修订记录
│   │   ├── proposals.md
│   │   ├── vague-words.txt
...
| [trae-harness-experiments](https://github.com/zzzkeepdd/trae-harness-experiments) | v1-v16 完整 A/B 实验数据 |
